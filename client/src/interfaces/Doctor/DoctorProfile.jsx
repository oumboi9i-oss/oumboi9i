...
export default function DoctorProfile({ doctorId, onNavigate, onUpdateUser }) {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/api/doctor/${doctorId}`)
      .then(r => r.json())
      .then(data => {
  ...
  const handleUpdate = async () => {
    setSaving(true);
    try {
      await fetch(`${process.env.REACT_APP_API_URL}/api/doctor/${doctorId}`, {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`${process.env.REACT_APP_API_URL}/api/doctor/${doctorId}`, { method: 'DELETE' });
      onNavigate?.('home');
