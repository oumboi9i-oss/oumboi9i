...
const NotificationBell = ({ currentUser, onNavigate }) => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/notification/user/${userId}`);
  ...
  const handleMarkAsRead = async (id) => {
    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/api/notification/${id}/read`);
      fetchNotifications();
    } catch (error) {
  ...
  const handleMarkAllAsRead = async () => {
    try {
      const userId = currentUser._id || currentUser.id;
      await axios.put(`${process.env.REACT_APP_API_URL}/api/notification/user/${userId}/read-all`);
      fetchNotifications();
    } catch (error) {
      console.error('Error:', error);
    }
  ...
  const acceptDemande = async (demandeId) => {
    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/api/demande/${demandeId}/accept`);
  ...
  const rejectDemande = async (demandeId) => {
    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/api/demande/${demandeId}/reject`);
