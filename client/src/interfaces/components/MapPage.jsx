...
export default function MapPage({ onNavigate, currentUser, role = "doctor" }) {
  ...
  const roleConfig = {
    doctor:      { color:"#2563eb", icon:"🧑‍⚕️", label:"Doctors",      api:`${process.env.REACT_APP_API_URL}/api/doctor/getAll`,     name:"fullName",     extra: d=>`🩺 ${d.specialty||""}` },
    nurse:       { color:"#10b981", icon:"👩‍⚕️", label:"Nurses",       api:`${process.env.REACT_APP_API_URL}/api/nurse/getAll`,      name:"userId",       extra: d=>`🎓 ${d.diplome||""}` },
    pharmacist:  { color:"#059669", icon:"💊",    label:"Pharmacists",  api:`${process.env.REACT_APP_API_URL}/api/pharmacist/getAll`, name:"nomPharmacie", extra: d=>`🏪 ${d.nomPharmacie||""}` },
    firefighter: { color:"#ef4444", icon:"🚒",    label:"Firefighters", api:`${process.env.REACT_APP_API_URL}/api/firefighter/getAll`,name:"matricule",    extra: d=>`⭐ ${d.grade||""}` },
  };
