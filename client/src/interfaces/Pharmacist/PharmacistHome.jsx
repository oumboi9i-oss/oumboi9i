...
export default function PharmacistHome({ onNavigate, currentUser }) {
  ...
  useEffect(() => {
    const fetchPharmacists = async () => {
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/api/pharmacist/getAll`);
    ...
    const fetchStats = async () => {
      try {
        const [gardesRes, demandesRes] = await Promise.all([
          fetch(`${process.env.REACT_APP_API_URL}/api/garde/getAll`).catch(() => null),
          fetch(`${process.env.REACT_APP_API_URL}/api/demande`).catch(() => null),
        ]);
